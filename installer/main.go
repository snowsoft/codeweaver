...
func installPrebuilt() {
	...
	if err != nil || resp.StatusCode != 200 {
		fmt.Println("❌ Failed to download binary")
...
func installBinary(binaryName string) {
	...
	case "windows":
		...
		case "1":
			...
			fmt.Printf("\n⚠️  Add %s to your PATH environment variable\n", userBin)
		case "2":
			installPath = filepath.Join(os.Getenv("SYSTEMROOT"), "System32", binaryName)
		default:
			fmt.Println("✅ Binary available in current directory")
			return
		}
	...
	if installPath != "" {
		if strings.Contains(installPath, "System32") || strings.HasPrefix(installPath, "/usr") {
			...
			fmt.Println("\n⚠️  Administrator/sudo access required")
...
func setupOnly() {
	fmt.Println("\n⚙️  Setting up configuration only...")
}
